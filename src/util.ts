...
export async function generateTitleImage(exec_cmd: string, title: string, outputPath: string, pic_dir: string) {
    try {
        // 执行 shell 命令 
        const { exec } = require('child_process');

        // 判断当前系统
        exec(`${exec_cmd} -text "${title}" -output "${outputPath}" -imageDir "${pic_dir}"`, (error: Error | null, stdout: string, stderr: string) => {
            if (error) {
                console.error(`执行 gen_pic_tool 出错: ${error}`);
                return;
            }
            if (stderr) {
                console.error(`gen_pic_tool stderr: ${stderr}`);
                return;
            }
            console.log(`gen_pic_tool 执行结果: ${stdout}`);
        });
